...
if [[ -z "${RV_FORT}" ]]; then
  ...
  mkdir build
  cd build
  cmake -DCMAKE_C_COMPILER="$RV_CC $RV_CFLAGS" -DCMAKE_CXX_COMPILER="$RV_CXX $RV_CXXFLAGS" -DCMAKE_Fortran_COMPILER="$RV_FORT $RV_FORTFLAGS" -DTrilinos_ENABLE_ALL_PACKAGES=ON -DCMAKE_INSTALL_PREFIX=$INSTALL_PATH ../
  make -j$MAX_THREADS
  if [ $? -ne 0 ]; then
    exit -1
  fi
