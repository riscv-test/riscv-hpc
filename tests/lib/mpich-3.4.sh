...
if [[ -z "${RV_FORT}" ]]; then
  ...
  CC="$RV_CC" CFLAGS="$RV_CFLAGS" CXX="$RV_CXX" CXXFLAGS="$RV_CXXFLAGS" ../configure --host=riscv64-unknown-linux --prefix=$INSTALL_PATH --disable-fortran --enable-cxx --with-device=ch3
  if [ $? -ne 0 ]; then
    exit -1
  fi
  make -j$MAX_THREADS
  ...
  FC="$RV_FORT" FCFLAGS="$RV_CFLAGS" CC="$RV_CC" CFLAGS="$RV_CFLAGS" CXX="$RV_CXX" CXXFLAGS="$RV_CXXFLAGS" ../configure --host=riscv64-unknown-linux --prefix=$INSTALL_PATH --enable-fortran --enable-cxx --with-device=ch3
  if [ $? -ne 0 ]; then
    exit -1
  fi
  make -j$MAX_THREADS
